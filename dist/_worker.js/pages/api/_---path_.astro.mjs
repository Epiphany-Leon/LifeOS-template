...
app.post("/expenses", async (c) => {
  try {
    ...
    const finalDate = date ? date.slice(0, 10) : (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
...
app.post("/projects", async (c) => {
  try {
    const DB = c.env.DB;
    const { title, status = "Planned", area, tags, cost = 0, date } = await c.req.json();
    const finalDate = date ? date.slice(0, 10) : (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
...
app.post("/todos", async (c) => {
  try {
    ...
    const finalDate = target_date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
...
app.post("/execution/eod-review", async (c) => {
  ...
  try {
    ...
    return c.json({ error: "AI 教练连接失败" }, 500);
  }
});
...
app.post("/knowledge/synthesize", async (c) => {
  ...
  try {
    ...
    const prompt = `你是我 LifeOS 的首席知识官。基于本周笔记生成复盘报告：1. 核心洞察 2. 认知盲区 3. 下周建议。笔记：${JSON.stringify(results.map((r) => ({ title: r.title, content: r.content.substring(0, 150) })))}`;
    const response = await fetch("[https://api.deepseek.com/chat/completions](https://api.deepseek.com/chat/completions)", {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
      body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "user", content: prompt }], temperature: 0.7 })
    ...
    return c.json({ error: "认知合成失败" }, 500);
  }
});
...
app.post("/vitals", async (c) => {
  try {
    ...
    const finalMood = mood_score !== void 0 ? mood_score : 5;
    const finalStress = stress_level !== void 0 ? stress_level : 30;
    await c.env.DB.prepare("INSERT INTO vitals (reflection, date, mood_score, stress_level) VALUES (?, ?, ?, ?)").bind(reflection, finalDate, finalMood, finalStress).run();
    return c.json({ success: true });
  } catch (e) {
    return c.json({ success: false }, 500);
  }
...
app.put("/vitals/:id", async (c) => {
  try {
    ...
    await c.env.DB.prepare("UPDATE vitals SET reflection = ?, mood_score = ?, stress_level = ? WHERE id = ?").bind(reflection, mood_score, stress_level, id).run();
    return c.json({ success: true });
  } catch (e) {
    return c.json({ success: false }, 500);
  }
...
app.post("/vitals/chat", async (c) => {
  ...
  try {
    ...
    const systemPrompt = `你是我（Lihong Gao）的灵魂导师。背景：JHU BAAI 研究生。准则：${principles.results.map((p) => p.content).join("; ")}。请给出理性且具启发性的对话。`;
    const response = await fetch("[https://api.deepseek.com/chat/completions](https://api.deepseek.com/chat/completions)", {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
